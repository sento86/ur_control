...
from ur_control.cfg import urConfig
...
class UR3control:
    def __init__(self, fc = 100.0, ns = 'ur3'):
        ...
        self.ns = ns
        self.pubTrajectory = rospy.Publisher("/arm_controller/command", JointTrajectory, queue_size=1)
        #self.pubTrajectory = rospy.Publisher(self.ns+"/arm_controller/command", JointTrajectory, queue_size=1)
        ...
        #self.home = np.array([0.0,-1.57,0.0,-1.57,3.14,3.14]).reshape((6,1))
        #self.home = np.array([0.0,-1.57,0.0,-3.14,3.14,3.14]).reshape((6,1))
        self.home = np.array([0.0,-3.14,0.0,-1.57,3.14,3.14]).reshape((6,1))
...
#         print self.kin.cart_inertia()
        ...
        rospy.on_shutdown(self.delete_gazebo_models)
        
        #param_srv = urConfig()
        srv = Server(urConfig, self.callback_param)
        
    def callback_param(self, config, level):
        #rospy.loginfo("""Reconfigure Request: {int_param}, {double_param}, {double_param},\ 
        #      {set_pose}, {frame}, {init_test}, {size}""".format(**config))
        if config.frame == 0:
            print "World frame"
            self.state_msg.reference_frame = 'world'
        elif config.frame == 1:
            print "Link 1 frame"
            self.state_msg.reference_frame = 'shoulder_link'
        elif config.frame == 2:
            print "Link 2 frame"
            self.state_msg.reference_frame = 'upper_arm_link'
        elif config.frame == 3:
            print "Link 3 frame"
            self.state_msg.reference_frame = 'forearm_link'
        elif config.frame == 4:
            print "Link 4 frame"
            self.state_msg.reference_frame = 'wrist_1_link'
        elif config.frame == 5:
            print "Link 5 frame"
            self.state_msg.reference_frame = 'wrist_2_link'
        elif config.frame == 6:
            print "Link 6 frame"
            self.state_msg.reference_frame = 'wrist_3_link'
        elif config.frame == 7:
            print "FT frame"
            self.state_msg.reference_frame = 'ft300_ft_measure'
            
        if config.reset_pose == True:
            self.reset_gazebo_models()
        else:
            if config.set_pose == True:
                self.state_msg.model_name = 'ball'
                self.state_msg.pose.position.x = config.x
                self.state_msg.pose.position.y = config.y
                self.state_msg.pose.position.z = config.z
                ...
                self.set_gazebo_models()
                
        if config.go_home == True:
            self.go_home()
                
        if config.run == True:
            self.u = self.q.copy()
            self.u[config.joint-1] = config.angle_final
            self.go_pose_timed(self.u, config.delay)
...
#         return self.kin.jacobian() * vel_joints
    ...
    def go_home(self, delay = 2.0):
        ...
        for i in range(0,iters):
            ...
            self.trajectory.header = Header(stamp=rospy.Time.now(), frame_id='base_link', seq=self.iter)
            self.trajectory.points[0] = JointTrajectoryPoint(positions = self.home, time_from_start = rospy.Duration(delay-i*self.Tc,0))
         
            self.pubTrajectory.publish(self.trajectory) # UR3 (simulated)
            #self.pubTrajectory_.publish(self.trajectory) # UR3 (real)
    ...
    def go_pose_timed(self, cmd, delay):
        
        self.iter+=1

        self.trajectory.header = Header(stamp=rospy.Time.now(), frame_id='base_link', seq=self.iter)
        self.trajectory.points[0] = JointTrajectoryPoint(positions = cmd, time_from_start = rospy.Duration(delay,0))
...
# #             print self.joint_positions
    ...
    def load_gazebo_models(self,
                           #ball_pose=Pose(position=Point(x=0.02, y=0.1, z=0.0)),
                           ball_pose=Pose(position=Point(x=0.1, y=-0.02, z=0.0)),
                           ball_reference_frame="ft300_ft_measure",
                           target_pose=Pose(position=Point(x=1, y=0, z=0)),
                           target_reference_frame="world"):
        # Get Models' Path
        model_path = rospkg.RosPack().get_path('ur_control')+"/models/"
        # Load Target SDF
        target_xml = ''
        ...
        try:
            ...
            resp_urdf = spawn_urdf("target", target_xml, "/",
                                   target_pose, target_reference_frame)
    ...
    def reset_gazebo_models(self):
        rospy.wait_for_service('/gazebo/set_model_state')
        try:
            self.state_msg.model_name = 'target'
            ...
            self.state_msg.model_name = 'ball'
            #self.state_msg.pose.position.x = 0.02
            #self.state_msg.pose.position.y = 0.1
            self.state_msg.pose.position.x = 0.1
            self.state_msg.pose.position.y = -0.02
    ...
    def delete_gazebo_models(self):
        ...
        try:
            ...
            resp_delete = delete_model("target")
        except rospy.ServiceException, e:
            rospy.loginfo("Delete Model service call failed: {0}".format(e))

def main():
    rospy.init_node("ur3_control")
